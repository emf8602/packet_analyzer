  ...
  def __init__(self, packet, number):
    ...
    if packet.haslayer("Ether"):
      ...
      if packet.haslayer("IP") or packet.haslayer("IPv6"):
        self.ip_layer = packet["IP"] if packet.haslayer("IP") else packet["IPv6"]
  ...
  def valid_packet(self, host, port, bool_filter, net):
    ...
    if port:
      ...
      if self.tcp_layer:
        ...
        return False
    if bool_filter and "ip" in bool_filter:
      if not self.ip_layer or self.ip_layer.version != 4:
        return False
    if bool_filter and "tcp" in bool_filter:
      if not self.tcp_layer:
        return False
    elif bool_filter and "udp" in bool_filter:
      if not self.udp_layer:
        return False
    elif bool_filter and "icmp" in bool_filter:
